 ...
 * (Kayıt Ara'nın tarih aralığı toplu Excel çıktısı) her kaydın satırları KENDİ
 * sayfalarına paylaştırılır (farklı kayıtların ürünleri asla aynı 13 satırlık sayfada
 * karışmaz); sayfa numaralandırması TÜM dosya boyunca sıralı devam eder (Sayfa 1, 2, 3...).
 * Tek kayıtlık çağrı `buildMalKabulWorkbook([{ receipt, items }], sablon)` şeklinde,
 * eski tek-kayıt davranışıyla birebir aynı çıktıyı üretir.
 *
...
export async function buildMalKabulWorkbook(receiptsWithItems, templateArrayBuffer) {
  ...
  for (const { receipt, items } of receiptsWithItems) {
    ...
    for (const pageItems of pages) {
      ...
      sheet.name = `Sayfa ${globalPageIndex + 1}`;
      ...
      const id = globalPageIndex + 1;
      sheet.id = id;
      sheet.orderNo = globalPageIndex;
