...
- **Task 4 (final review'dan) — çözülecek bulgular:**
  1. **Kritik:** `src/lib/auth.js`'teki `getCurrentProfile()` hiç önbelleklenmiyor (Task 1'in `cacheAside`'ı sadece `companies`/`products`'ı sarmalıyor). Sonuç: uygulama çevrimdışıyken soğuk açılış (`main.js` → `getCurrentProfile()`) veya herhangi bir sayfa geçişi (her route handler kendi `getCurrentProfile()` çağrısını yapıyor) doğrudan "Bir hata oluştu" ekranına düşüyor — mal kabul formuna hiç ulaşılamıyor. Tek çalışan senaryo, kullanıcı zaten `/yeni-kabul` sayfasındayken bağlantı kesilmesi. Düzeltme: `getCurrentProfile()` içindeki `profiles` select'i `cacheAside`'a sarmalanmalı, anahtar kullanıcıya özel olmalı (örn. `` `cache:profile:${session.user.id}` ``) — `profiles_select_all` zaten `using (true)` olduğu için önbelleklenen veri kullanıcının zaten okuyabileceği bir şey.
  2. **Önemli:** `cacheAside`'daki `localStorage.setItem` başarısız olursa (quota aşımı, Safari private mode) BAŞARILI bir fetch sonucu bile hataya dönüşüyor. `setItem` çağrısı ayrı bir `try/catch` içine alınmalı ("cache yazımı best-effort'tur, başarısız olsa da sonucu dön").
  3. **Önemli:** Kuyruktaki kayıtlar sadece `online` event'inde tekrar denenir; ama `navigator.onLine` arayüzün "yukarıda" olduğunu gösterir, gerçek erişilebilirliği değil — zayıf ama kesintisiz depo wifi'sinde fetch başarısız olur, kayıt kuyruğa girer, ama `offline`/`online` event çifti hiç tetiklenmediği için asla tekrar denenmez. `main.js`'e `setInterval(trySync, 30000)` gibi periyodik bir yedek tetikleyici eklenmeli.
  4. **Önemli:** `syncQueuedReceipts`, `save()`'in aksine ağ hatası/uygulama hatası ayrımı yapmıyor — her hatayı aynı şekilde `failed`/`attempts++` sayıyor. Kalıcı bir uygulama hatası (örn. senkron öncesi silinmiş bir firma/ürün → FK ihlali) sonsuza kadar aynı şekilde yeniden denenir. `lastError` alanı zaten saklanıyor ama hiçbir yerde KULLANICIYA gösterilmiyor. `offline-banner.js`, bekleyen kayıt sayısının yanında (varsa) en son hatayı da özet olarak göstermeli — en azından "N kayıt senkronize edilecek (son hata: ...)" şeklinde, kullanıcının sıkışmış bir kaydı fark edebilmesi için.
  5. **Önemli:** `vite.config.js`'teki `registerType: 'autoUpdate'`, yeni bir deploy geldiğinde formu ortasında dolduran bir kullanıcının sayfasını habersizce yeniler (state kaybı). `registerType: 'prompt'`a geçilip `main.js`'e basit bir "Yeni sürüm mevcut, yenile" bildirimi eklenmeli (`registerSW({ onNeedRefresh() { ... } })`).
  6. **Önemli:** `offline-banner.js` ve `cacheAside`'ın cache-yazım-hatası yolu için hiç test yok; `tests/offline-banner.test.js` (varsa `offline-queue.test.js`'teki mock desenini takip ederek) ve `offline-cache.test.js`'e bir "setItem atarsa yine de sonucu döner" testi eklenmeli.
