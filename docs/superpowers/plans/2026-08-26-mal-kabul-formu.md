...
- **Atomiklik (Task 1 review'dan):** `createReceiptWithItems`, `receipts` satırını ve `receipt_items` satırlarını iki ayrı Supabase çağrısı yerine `supabase/migrations/0006_create_receipt_rpc.sql`'deki `create_receipt_with_items` Postgres fonksiyonu (RPC) üzerinden **tek bir atomik işlemde** oluşturur. Böylece satır ekleme başarısız olursa receipt satırı da otomatik geri alınır (öksüz taslak kaydı riski ortadan kalkar). Fonksiyon `security invoker` ile tanımlıdır — RLS politikaları çağıran kullanıcı için aynen uygulanmaya devam eder, sadece atomiklik eklenir.
...
- **Sağlamlaştırma (final review'dan, `supabase/migrations/0007_receipt_hardening.sql`):**
  - `create_receipt_with_items` RPC'si artık `p_submit_to_quality boolean default false` parametresi alır ve tek çağrıda hem receipt+satırları oluşturur hem de (istenirse) `kalite_bekliyor`'a geçirir — böylece "Kaydet ve Kalite Onayına Gönder" tek bir atomik işlem olur (önceki haliyle create+submit iki ayrı çağrıydı, ikincisi başarısız olursa öksüz taslak kalıyordu). RPC ayrıca `p_items` boşsa veritabanı seviyesinde reddeder.
  - `receipts` tablosuna bir `before update` trigger eklendi: `status`'u `onaylandi`'ye çeken bir UPDATE, ilgili `receipt_items`'ın hepsi `uygun`/`uygun_degil` işaretlenmeden ve en az bir satır olmadan veritabanı seviyesinde reddedilir — `finalizeQuality`'nin uygulama seviyesindeki kontrolü artık RLS/trigger ile birlikte iki katmanlı.
  - `receipt_items.quantity` için `check (quantity > 0)` eklendi.
  - `createReceiptWithItems` artık isteğe bağlı bir `clientUuid` parametresi kabul eder (verilmezse `crypto.randomUUID()` üretilir) — Plan 5'in offline kuyruğu tekrar oynatma (replay) sırasında aynı UUID'yi yeniden kullanarak idempotency sağlayabilsin diye.
  - `src/lib/receipts.js`'teki `submitForQuality`, `updateItemUygunluk`, `finalizeQuality` artık her UPDATE'den sonra `.select()` ile etkilenen satırı kontrol eder ve 0 satır dönerse hata fırlatır (RLS'in sessizce 0 satır güncellemesi — örn. kayıt başka biri tarafından zaten işlenmiş — artık "başarılı" gibi görünmez).
  - `src/pages/kalite-onay.js`'teki satır bazlı `uygunluk`/`not` güncellemeleri artık `await` ile beklenip hatası yakalanıyor ve kullanıcıya gösteriliyor (önceden "fire-and-forget" idi, ağ hatasında sessizce kayboluyordu).
  - `/yeni-kabul` rotası ve nav linki artık `/kalite-onay` ile aynı şekilde role bazlı gizleniyor (sadece `depo_yonetici`).
  - `src/pages/yeni-kabul.js`'teki "Kaydet"/"Kaydet ve Kalite Onayına Gönder" butonları kaydetme sırasında devre dışı bırakılır (çift gönderim engeli) ve miktarı 0 veya negatif olan satırlarla kaydetmeye izin verilmez.
